...
public class DBMgr {
    ...
    public PlayerData QueryPlayerData(string acct, string pass) {
        ...
        bool isNew = true;  //是否是新账号
        PlayerData playerData = null;
        MySqlDataReader reader = null;
        //数据库操作容易出错，使用try catch
        try {
            ...
            if (reader.Read()) {
                ...
                if (_pass.Equals(pass)) {
                    ...
                    #region Task Arr
                    //数据示意：1|0|0#2|0|0#3|0|0#    任务id|进度|是否领取
    ...
    private int InsertNewAcctData(string acct, string pass, PlayerData pd) {
        int id = -1;
        try {
            MySqlCommand cmd = new MySqlCommand("insert into account set acct=@acct,pass =@pass,name=@name,level=@level,exp=@exp,power=@power,coin=@coin,diamond=@diamond," +
            ...
            //1|0|0#1|0|0#1|0|0#    任务id|进度|是否领取
            string taskInfo = "";
